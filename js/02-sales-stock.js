...
// ==========================================
    ...
    window.__pendingProductImageStoragePath = null;
      ...
      // Debounced re-render: typing fires this on every keystroke, but the actual
      // table rebuild (rebuilding <thead>/<tbody> HTML for potentially thousands of
      // rows) only needs to run once the person pauses typing for a moment.
      window.__debouncedRenderStock = window.debounce ? window.debounce(() => window.renderStock(), 250) : window.renderStock;

      window.__setStockFilter = function(key, value) {
        stockFilters[key] = value;
        window.stockCurrentPage = 1; // Reset to page 1 on search
        window.__debouncedRenderStock();
      };
