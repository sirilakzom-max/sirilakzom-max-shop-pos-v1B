...
window.saveSupabaseConfig = async function () {
  ...
  const pwEl = document.getElementById('supabase-auth-password');
  ...
  if (!url || !key || !email || !password) {
    return showAlert('ข้อมูลไม่ครบ', 'กรุณากรอก Project URL, anon public key, อีเมล และรหัสผ่านของบัญชีนี้', true);
  }
  if (!/^https:\/\/.+\.supabase\.co$/.test(url)) return showAlert('URL ไม่ถูกต้อง', 'รูปแบบ Project URL ควรเป็น https://xxxxx.supabase.co', true);
  if (!/^\S+@\S+\.\S+$/.test(email)) return showAlert('อีเมลไม่ถูกต้อง', 'กรุณาใช้อีเมลที่ใช้งานได้สำหรับ Supabase Auth และกู้คืนบัญชี', true);
  if (password.length < 8) return showAlert('รหัสผ่านไม่ถูกต้อง', 'รหัสผ่านต้องมีอย่างน้อย 8 ตัวอักษร', true);
  if (/service_role|sb_secret/i.test(key)) return showAlert('Key ไม่ปลอดภัย', 'ห้ามใช้ service_role/secret key ให้ใช้ anon public key เท่านั้น', true);

  try {
    const accountId = localStorage.getItem('POS_ACCOUNT_ID');
    if (!accountId) return showAlert('ยังไม่มีบัญชี', 'กรุณาสร้างบัญชีผู้ใช้ก่อนเชื่อมต่อฐานข้อมูล', true);

    const client = window.supabase.createClient(url, key);
    _supabaseClient = client;

    // ตั้งค่าเป็นรายบัญชี แต่ยังไม่ถือว่าเชื่อมสำเร็จจนกว่า Auth + RLS จะผ่าน
    setConfiguredSupabase(url, key, accountId);
    localStorage.setItem('POS_SUPABASE_AUTH_EMAIL::' + accountId, email);
    localStorage.setItem('POS_SUPABASE_AUTH_EMAIL', email);
    ...
    if (!authResult.ok) {
      localStorage.removeItem(getAccountScopedKey(SUPABASE_URL_STORAGE_KEY, accountId));
      localStorage.removeItem(getAccountScopedKey(SUPABASE_KEY_STORAGE_KEY, accountId));
      localStorage.removeItem(SUPABASE_URL_STORAGE_KEY);
      localStorage.removeItem(SUPABASE_KEY_STORAGE_KEY);
      _supabaseClient = null;
      if (authResult.reason === 'email_confirmation_required') {
        return showAlert('ต้องยืนยันอีเมลก่อน', 'Supabase เปิด Confirm email อยู่ จึงยังไม่มี session สำหรับ RLS ให้ปิด Confirm email ชั่วคราวสำหรับการทดสอบ หรือยืนยันอีเมลก่อน แล้วจึงเชื่อมต่ออีกครั้ง', true);
      }
      return showAlert('Supabase Auth ไม่สำเร็จ', String(authResult.reason || 'ไม่สามารถยืนยันบัญชีได้'), true);
    }

    const authUserId = authResult.user.id;
    // A Supabase Project is one store. Ensure the authenticated owner has a store identity
    // and membership before any business-table read/write occurs.
    const { error: storeErr } = await client.rpc('create_pos_account', { p_store_name: db.storeName || 'ร้านค้า', p_account_code: accountId });
    ...
    const currentBound = localStorage.getItem('POS_BOUND_SUPABASE_URL');
    const sameProject = currentBound === url;

    if (remote && remote.data) {
      // Store isolation is enforced by store_members + RLS, not by the current Auth user id.
      const ok = confirm('พบข้อมูลอยู่ใน Supabase นี้แล้ว\n\nกด OK = ใช้ข้อมูลจากฐานข้อมูลนี้และไม่ส่งข้อมูลในเครื่องขึ้นไปทับ\nกด Cancel = ยกเลิกการเชื่อมต่อ');
      if (!ok) return;
      db = JSON.parse(JSON.stringify(remote.data));
      window.db = db;
      await localforage.setItem(getAccountDbKey(), db);
      localStorage.setItem(LAST_SYNCED_KEY, remote.updated_at || new Date().toISOString());
    } else {
      const ok = confirm('ฐานข้อมูลนี้ยังว่าง\n\nกด OK = นำข้อมูลปัจจุบันในเครื่องขึ้นเป็นข้อมูลเริ่มต้นของ Project นี้\nกด Cancel = เชื่อมต่อแบบยังไม่ส่งข้อมูล');
      if (ok) {
        const { error: seedErr } = await client.from('pos_state').upsert({
          id: POS_STATE_ROW_ID,
          owner_id: authUserId,
    ...
    localStorage.setItem('POS_BOUND_SUPABASE_URL', url);
    localStorage.setItem('POS_SUPABASE_AUTH_USER_ID', authUserId);
    if (!sameProject) localStorage.removeItem(LAST_SYNCED_KEY);
    _supabaseClient = client;
    await localforage.setItem(getAccountDbKey(), db);
    if (pwEl) pwEl.value = '';
    showAlert('เชื่อมต่อฐานข้อมูลสำเร็จ', 'Supabase Auth + RLS ผ่านแล้ว บัญชีนี้ผูกกับ Project นี้และจะไม่ใช้ฐานข้อมูลของร้านอื่น', false);
    updateSyncStatusBadge('synced', localStorage.getItem(LAST_SYNCED_KEY));
  } catch (err) {
    console.error('Supabase connection test failed:', err);
    showAlert('เชื่อมต่อไม่สำเร็จ', 'ตรวจสอบ SQL/RLS, Project URL, anon public key และ Supabase Auth แล้วลองใหม่\n\n' + (err.message || err), true);
  }
};
...
window.loadSupabaseConfigIntoForm = function () {
  ...
  const emailEl = document.getElementById('supabase-auth-email');
  const accountId = localStorage.getItem('POS_ACCOUNT_ID') || '';
  if (emailEl) emailEl.value = localStorage.getItem('POS_SUPABASE_AUTH_EMAIL::' + accountId) || '';
  const pwEl = document.getElementById('supabase-auth-password');
  if (pwEl) pwEl.value = '';
};
        ...
        if (remoteTime > localKnownTime + 1000) {
          ...
          if (typeof window.showCustomConfirm === 'function') {
            window.showCustomConfirm(
              ...
              "พบว่ามีเครื่องอื่นอัปเดตข้อมูลขึ้นระบบขณะที่คุณกำลังใช้งาน และระบบไม่สามารถรวมข้อมูลอัตโนมัติได้ คุณต้องการบังคับซิงค์ (Force Sync) ข้อมูลในเครื่องนี้ขึ้นไปทับหรือไม่? (ข้อมูลของอีกเครื่องจะถูกเขียนทับ)",
              async () => {
            ...
            window.showAlert(
              ...
              "มีเครื่องอื่นอัปเดตข้อมูลขึ้นระบบก่อนหน้า และระบบไม่สามารถรวมข้อมูลอัตโนมัติได้ กรุณากดปุ่ม 'บังคับซิงค์' หากต้องการเขียนทับ",
              true
            );
          }
          return false;
