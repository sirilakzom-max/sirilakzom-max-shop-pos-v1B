...
// ==========================================
      ...
      window.changePinFromSettings = async function() {
        ...
        if(db.pinHash && curHash !== db.pinHash) { const legacy = typeof hashPINLegacy === 'function' ? await hashPINLegacy(cur, db.pinSalt) : ''; if (legacy !== db.pinHash) return showAlert("ผิดพลาด", "รหัส PIN ปัจจุบันไม่ถูกต้อง", true); }
