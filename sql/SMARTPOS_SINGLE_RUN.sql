...
-- ============================================================
-- COMPATIBILITY v4: user_id / identity column safety
...
-- ============================================================
-- COMPATIBILITY: existing/partially-installed databases
-- CREATE TABLE IF NOT EXISTS does NOT add missing columns.
-- PATCH 3: this whole block used to sit BEFORE the "create table" for
-- bills/bill_items above (lines ~245-291 in the original file). On a
-- genuinely fresh install neither table existed yet at that point, so
-- "update public.bills ..." (which has no "if exists" guard, unlike the
-- "alter table if exists" statements around it) failed immediately with
-- "relation public.bills does not exist" and stopped the whole script.
-- Moved here — after both tables are guaranteed to exist — fixes that.
-- ============================================================
...
-- Existing bills may have created_at but no sold_at.
update public.bills
...
create index if not exists idx_bills_store_date
  on public.bills(store_id, sold_at desc);

-- Bill items are also used by the atomic checkout path.
...
-- ============================================================
-- COMPATIBILITY v5: audit_log.created_by safety
-- ============================================================
...
alter table if exists public.audit_log
  ...
  add column if not exists created_at timestamptz,
  -- PATCH: a genuinely fresh install only ever runs the "create table if not
  -- exists" a few lines above (id uuid / user_id / ...), which does NOT
  -- include these 4 columns. The later "create table if not exists" (with
  -- owner_id/ts/actor/device_id) further down is then skipped as a no-op
  -- because the table already exists — so on a brand-new Supabase project
  -- these columns never get created. That breaks the "audit_log_select" /
  -- "audit_log_insert" RLS policies below (they reference owner_id) and
  -- breaks every logTransaction() insert from the app (which writes
  -- owner_id/ts/actor/device_id). Adding them explicitly here fixes both.
  add column if not exists owner_id uuid references auth.users(id) on delete cascade,
  add column if not exists ts timestamptz not null default now(),
  add column if not exists actor text,
  add column if not exists device_id text;

-- Backfill owner_id/ts on any rows that existed before this patch, so the
-- policy below (auth.uid() = owner_id) still works for old audit entries.
update public.audit_log
set owner_id = coalesce(owner_id, user_id, created_by),
    ts = coalesce(ts, created_at)
where owner_id is null or ts is null;

-- PATCH 2 (critical): audit_log.id was created as type "uuid" by the very
-- first "create table if not exists" above. But every write the app makes
-- uses a prefixed id like 'AL' + hex (client logTransaction) or
-- 'AL-' || gen_random_uuid()::text (the process_sale_atomic RPC further
-- down this file) — neither is valid "uuid" syntax (the letter L isn't a
-- hex digit), so every insert into audit_log fails with
-- "invalid input syntax for type uuid". For process_sale_atomic specifically
-- this insert happens inside the SAME function as the sale itself, so that
-- failure rolls back the whole checkout — stock is never decremented and no
-- bill is created, even though the app told the customer the sale succeeded.
-- No foreign key references audit_log.id, so this is safe to change.
...
-- ============================================================
-- COMPATIBILITY v6: error_logs.created_by safety
-- ============================================================
create table if not exists public.error_logs (
  id text primary key,
  owner_id uuid,
...
-- ============================================================
-- PATCH 4: sale_transactions idempotency should be scoped per STORE, not
-- per individual Supabase Auth user. As originally created, the unique
-- constraint was unique(owner_id, idempotency_key) — so two different
-- members of the SAME store (owner + a staff member, each with their own
-- auth.uid()) could theoretically submit the same idempotency_key without
-- the database catching it as a duplicate, since the constraint only
-- guards uniqueness within a single owner_id. process_sale_atomic's actual
-- duplicate-sale check already queries by store_id + idempotency_key (see
-- further down this file), so the unique constraint should match that
-- scope. store_id is guaranteed to exist and be backfilled by this point
-- (the loop right above adds + backfills it on every business table,
-- sale_transactions included), so it's safe to reference here.
-- Safe to re-run.
-- ============================================================
do $$
declare c text;
begin
  ...
  where tc.table_schema = 'public'
    and tc.table_name = 'sale_transactions'
    ...
    ) = array['idempotency_key','owner_id']::text[]
  limit 1;

  if c is not null then
    execute format('alter table public.sale_transactions drop constraint %I', c);
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'sale_transactions_store_idem_key'
      and conrelid = 'public.sale_transactions'::regclass
  ) then
    execute 'alter table public.sale_transactions add constraint sale_transactions_store_idem_key unique (store_id, idempotency_key)';
  end if;
end $$;
...
drop policy if exists audit_log_select on public.audit_log;
drop policy if exists audit_log_insert on public.audit_log;
drop policy if exists audit_log_store_select on public.audit_log;
...
drop policy if exists audit_log_store_insert on public.audit_log;
...
drop policy if exists error_logs_select on public.error_logs;
drop policy if exists error_logs_insert on public.error_logs;
drop policy if exists error_logs_store_select on public.error_logs;
...
drop policy if exists error_logs_store_insert on public.error_logs;
...
drop policy if exists app_accounts_own_insert on public.app_accounts;
drop policy if exists app_accounts_own_update on public.app_accounts;
drop policy if exists app_accounts_store_select on public.app_accounts;
...
drop policy if exists app_accounts_owner_update on public.app_accounts;
...
drop policy if exists app_accounts_own_insert on public.app_accounts;
drop policy if exists app_accounts_owner_insert on public.app_accounts;
...
drop policy if exists product_images_own_insert on storage.objects;
drop policy if exists product_images_own_delete on storage.objects;
drop policy if exists product_images_store_select on storage.objects;
...
drop policy if exists documents_own_insert on storage.objects;
drop policy if exists documents_own_delete on storage.objects;
drop policy if exists documents_store_select on storage.objects;
...
drop policy if exists documents_store_insert on storage.objects;
...
drop policy if exists documents_store_delete on storage.objects;
